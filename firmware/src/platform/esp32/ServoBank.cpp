...
bool ServoBank::pressForce(int index, double intensity) {
    if (index < 0 || index >= (int)servos_.size()) return false;
    // Velocity / expression shapes the contact force between rest (lifted) and
    // active (full force). The maths lives in servoForceTargetUs (unit-tested).
    bool ok = writeMicros(index, servoForceTargetUs(servos_[index], intensity));
    rt_[index].mode = Mode::Active;  // held, not a strike-and-return
    return ok;
}

...
const char* signalKindName(SignalKind k) {
    switch (k) {
        ...
        case SignalKind::BowPwm: return "bowPwm";
        case SignalKind::BowDir: return "bowDir";
        case SignalKind::BowEnable: return "bowEnable";
...
void ProfileStorage::toJson(const Profile& p, JsonDocument& doc) {
    ...
    for (const auto& s : p.servos) {
        ...
        o["minForceUs"] = s.minForceUs;
    }

    JsonArray bows = doc["bows"].to<JsonArray>();
    for (const auto& b : p.bows) {
        JsonObject o = bows.add<JsonObject>();
        o["enabled"] = b.enabled;
        o["stringIndex"] = b.stringIndex;
        o["minDutyPct"] = b.minDutyPct;
        o["maxDutyPct"] = b.maxDutyPct;
        o["pwmFreqHz"] = b.pwmFreqHz;
        o["spinUpMs"] = b.spinUpMs;
        o["spinDownMs"] = b.spinDownMs;
        o["reverse"] = b.reverse;
        o["alternate"] = b.alternate;
    }
}
...
bool ProfileStorage::fromJson(JsonVariantConst doc, Profile& out) {
    ...
    out.project = doc["project"] | "Servo-bowed-strings-GMB";
    ...
    out.network.apSsid = net["apSsid"] | "Servo-bowed-strings-GMB";
    ...
    for (JsonObjectConst o : doc["servos"].as<JsonArrayConst>()) {
        ...
        s.minForceUs = o["minForceUs"] | 0;
        out.servos.push_back(s);
    }

    out.bows.clear();
    for (JsonObjectConst o : doc["bows"].as<JsonArrayConst>()) {
        BowConfig b;
        b.enabled = o["enabled"] | false;
        b.stringIndex = o["stringIndex"] | -1;
        b.minDutyPct = o["minDutyPct"] | 25;
        b.maxDutyPct = o["maxDutyPct"] | 100;
        b.pwmFreqHz = o["pwmFreqHz"] | 20000;
        b.spinUpMs = o["spinUpMs"] | 40;
        b.spinDownMs = o["spinDownMs"] | 60;
        b.reverse = o["reverse"] | false;
        b.alternate = o["alternate"] | false;
        out.bows.push_back(b);
    }

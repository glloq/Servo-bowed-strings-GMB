...
SignalKind signalKindFromName(const std::string& signal) {
    // Bow (H-bridge) signals are checked first: "BOW_DIR1" must not fall through
    // to the generic "DIR" rule, and "BOW_EN" must not match "ENABLE".
    if (startsWith(signal, "BOW_PWM")) return SignalKind::BowPwm;
    if (startsWith(signal, "BOW_DIR")) return SignalKind::BowDir;
    if (startsWith(signal, "BOW_EN")) return SignalKind::BowEnable;
...
bool PinManager::autoAssign(const PinRequest& req) {
    ...
    // Recommended assignment table (spec 11.5). Four strings max on a bowed
    // instrument, so four entries per stepper signal are enough.
    ...
    // Bow H-bridge lines: one PWM (speed) and one DIR per motor, one shared EN.
    const std::vector<int8_t> bowPwmPref = {1, 2, 10, 11};
    const std::vector<int8_t> bowDirPref = {15, 16, 38, 39};
    ...
    if (req.bowMotors) {
        for (int i = 0; i < n; ++i) {
            ok &= place("BOW_PWM" + std::to_string(i + 1), SignalKind::BowPwm,
                        {bowPwmPref[i]});
            ok &= place("BOW_DIR" + std::to_string(i + 1), SignalKind::BowDir,
                        {bowDirPref[i]});
        }
        // A single ENABLE / STBY line disables every bow H-bridge at once (spec
        // §21.2). GPIO33 is a caution pin on some module variants but is the only
        // free recommended-class output left once four strings are wired.
        ok &= place("BOW_EN", SignalKind::BowEnable, {33});
    }
    return ok;
}

...
//
// A bowed string sounds CONTINUOUSLY while the note is held: reaching the fret
// starts the bow (motor + descent servo) and it keeps sounding until Note Off,
// which stops the bow and lifts it. This differs from a plucked string, whose
// note is a one-shot strike that then decays on its own.
#pragma once
...
public:
    ...
    // action by advancing the command id. The bow is armed automatically once
    // the string is ready.
    ...
    // Start bowing (motor on, bow lowered). Runs only when `id` still matches the
    // current command and the string is ready — otherwise it is silently ignored.
    ...
    // True for a normal note that arms its bow as soon as it is ready; false
    // while a note is only PREPARED (anticipated) and still waiting for trigger().
    // Lets the scheduler skip bow-lead anticipation for a merely-prepared note
    ...
    // Release the note. Cancels any armed/prepared attack; a sounding note stops
    // (bow off + lifted).
    void noteOff();

    // Bow fully stopped/lifted -> back to idle.
    void stoppingDone();

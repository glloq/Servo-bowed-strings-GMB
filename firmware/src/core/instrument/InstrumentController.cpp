...
void InstrumentController::refreshSoundingIntensity() {
    // Only strings actually sounding (active and not merely prepared) follow
    // CC7/CC11 live: a bowed note is modulated while held. The scheduler reads
    // the updated target intensity every tick and re-drives bow speed + force.
    for (size_t i = 0; i < targets_.size(); ++i) {
        if (targets_[i].active && preparedId_[i] == 0)
            targets_[i].intensity =
                applyVelocityCurve(velocityCurve_, targets_[i].velocity) * attackGain();
    }
}
...
void InstrumentController::handleEvent(const MidiEvent& e, uint32_t nowUs) {
    ...
    if (e.isControlChange()) {
        ...
        if (e.data1 == 7) {  // channel volume -> expression gain (live)
            volume_ = e.data2 / 127.0;
            refreshSoundingIntensity();  // modulate held notes, not just the next
            return;
        }
        if (e.data1 == 11) {  // expression -> expression gain (live)
            expression_ = e.data2 / 127.0;
            refreshSoundingIntensity();
            return;
        }

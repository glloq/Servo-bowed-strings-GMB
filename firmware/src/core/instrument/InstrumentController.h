...
private:
    ...
    // Global expression gain from CC7 (volume) and CC11 (expression), 0..1 each.
    // A bowed string sounds continuously, so — unlike a plucked one — these
    // modulate the note WHILE it is held: changing either re-derives the live
    // intensity of every sounding string (bow speed + contact force), giving
    // crescendo / decrescendo and phrasing (spec section 7).
    double volume_ = 1.0;
    double expression_ = 1.0;
    double attackGain() const { return volume_ * expression_; }
    // Recompute the live intensity of every currently-sounding string from its
    // stored velocity and the current expression gain (CC7/CC11 modulation).
    void refreshSoundingIntensity();

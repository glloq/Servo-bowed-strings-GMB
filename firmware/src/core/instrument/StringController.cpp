...
void StringController::noteOff() {
    ...
    // Any attack still in preparation is cancelled; a sounding note is stopped
    // (bow off + lifted).
    bool wasSounding = state_ == StringState::Bowing;
    invalidate();  // kills any armed bow (spec: no delayed attack after Note Off)
    state_ = wasSounding ? StringState::Stopping : StringState::Idle;
}

void StringController::stoppingDone() {
    if (state_ == StringState::Stopping) state_ = StringState::Idle;
}

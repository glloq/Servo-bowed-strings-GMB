// Servo-bowed-strings-GMB — core shared types and constants.
//
...
// Hard capacity limits derived from the spec (section 6). A bowed instrument is
// intentionally capped at four strings (violin / viola / cello / double bass).
constexpr uint8_t kMaxStrings = 4;      // 1..4 strings / stepper axes / fingers
constexpr uint8_t kMaxBowMotors = 4;    // one DC bow motor (H-bridge) per string
constexpr uint8_t kMaxServoOutputs = 16; // PCA9685 channels
constexpr uint8_t kMaxLedcChannels = 8;  // ESP32-S3 LEDC channels (bow PWM + direct servos)

...
std::vector<ValidationIssue> ProfileValidator::validate(const Profile& p) {
    ...
    // Capacity: 1..4 strings, one stepper axis, one finger and one bow per string.
    ...
    // Does string `i` have an enabled bow motor?
    auto hasBow = [&](int strIdx) {
        for (const auto& b : p.bows)
            if (b.enabled && b.stringIndex == strIdx) return true;
        return false;
    };
    int enabledBows = 0;
    for (const auto& b : p.bows) if (b.enabled) ++enabledBows;
    ...
    for (size_t i = 0; i < p.strings.size(); ++i) {
        ...
        // A bowed string needs its H-bridge PWM (speed) and DIR (direction) lines.
        if (hasBow(static_cast<int>(i))) {
            if (!hasPin("BOW_PWM" + n))
                err("pins.BOW_PWM" + n, "Bow motor PWM pin is required for this string");
            if (!hasPin("BOW_DIR" + n))
                err("pins.BOW_DIR" + n, "Bow motor DIR pin is required for this string");
        }
    }
    if (!p.strings.empty() && !hasPin("ENABLE"))
        err("pins.ENABLE", "A driver ENABLE pin is required");
    // One shared ENABLE / STBY line disables every bow H-bridge at once (§21.2).
    if (enabledBows > 0 && !hasPin("BOW_EN"))
        err("pins.BOW_EN", "A shared bow-motor ENABLE (BOW_EN) pin is required");
    if (anyPca) {
    ...
    // ESP32-S3 has 8 LEDC channels, shared by direct-GPIO servos and bow-motor
    // PWM outputs (the steppers use the RMT engine, not LEDC).
    if (directServos + enabledBows > kMaxLedcChannels)
        err("ledc.channels",
            "Too many LEDC users: direct-GPIO servos + bow motors must be <= 8 "
            "(ESP32-S3 has 8 LEDC channels)");
    ...
    // Servo configuration: PCA vs direct GPIO, with or without a PCA9685.
    {
        ...
        for (size_t i = 0; i < p.servos.size(); ++i) {
            ...
            if (s.minForceUs != 0 &&
                (s.minForceUs < s.pulseMinUs || s.minForceUs > s.pulseMaxUs))
                err(tag + ".minForceUs",
                    "Minimum contact-force pulse is outside the servo's min/max range");
    ...
    // Bow-drive presence (spec §5.3): the string is set in vibration by a DC bow
    // motor pressed on by the bowPress descent servo.
    {
        ...
        // Every enabled string needs its own bow motor and its bowPress descent
        // servo — without either it can never sound.
        for (size_t i = 0; i < p.strings.size(); ++i) {
            if (!p.strings[i].enabled) continue;
            if (!hasBow(static_cast<int>(i)))
                err("bows",
                    "String " + std::to_string(i) +
                        " needs a bow motor to be sounded");
            if (!hasServoRole("bowPress", static_cast<int>(i)))
                err("servos.bowPress",
                    "String " + std::to_string(i) +
                        " needs a bowPress (bow descent) servo");
        }
        // A fretted string (maxFret > 0) MUST have a finger servo: without one the
        // scheduler would treat every note as an open string and bow a wrong
    ...
    // Bow-motor (H-bridge) configuration.
    {
        std::vector<int> bowForString(p.strings.size(), 0);
        for (size_t i = 0; i < p.bows.size(); ++i) {
            const BowConfig& b = p.bows[i];
            if (!b.enabled) continue;
            std::string tag = "bows[" + std::to_string(i) + "]";
            if (b.stringIndex < 0 || b.stringIndex >= (int)p.strings.size()) {
                err(tag + ".stringIndex", "Bow references a string that does not exist");
            } else {
                if (++bowForString[b.stringIndex] > 1)
                    err(tag + ".stringIndex",
                        "String " + std::to_string(b.stringIndex) +
                            " has more than one bow motor");
            }
            if (b.minDutyPct > b.maxDutyPct)
                err(tag + ".duty", "minDutyPct must be <= maxDutyPct");
            if (b.maxDutyPct > 100) err(tag + ".maxDutyPct", "maxDutyPct must be 0..100");
            // The LEDC clock/divider must be able to produce the requested PWM
            // frequency at a usable resolution; keep it in a sane audio-silent band.
            if (b.pwmFreqHz < 100 || b.pwmFreqHz > 40000)
                err(tag + ".pwmFreqHz", "Bow PWM frequency must be 100..40000 Hz");
        }
    }

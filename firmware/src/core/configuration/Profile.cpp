...
Profile Profile::makeDefault(const std::string& name, uint8_t stringCount,
    ...
    for (uint8_t i = 0; i < stringCount; ++i) {
        AxisConfig axis;
        axis.openNote = i < tuning.size() ? tuning[i] : 55;
        ...
        ServoConfig bowPress;
        bowPress.enabled = true;
        bowPress.function = "bowPress";
        bowPress.stringIndex = static_cast<int8_t>(i);
        bowPress.source = ServoSource::Pca;
        bowPress.channel = static_cast<uint8_t>(4 + i);  // channels 4..7 : bow descent
        bowPress.minForceUs = 1300;     // a soft note still contacts the string
        p.servos.push_back(bowPress);

        BowConfig bow;
        bow.enabled = true;
        bow.stringIndex = static_cast<int8_t>(i);
        p.bows.push_back(bow);
    }
    ...
    // Automatic pin assignment for the reference board (incl. bow H-bridge lines).

...
#include "platform/esp32/BowBank.h"
...
// Per-string non-blocking playback scheduler. A bowed string moves to the fret,
// presses the finger, then lowers the bow (descent servo) and spins the bow motor
// — and keeps bowing until the note is released.
struct StringSched {
    ...
    int fingerIndex = -1;
    int bowPressIndex = -1;     // bow descent servo (contact force) for this string
    int bowMotorIndex = -1;     // H-bridge bow motor index for this string
    uint32_t moveHoldUntilMs = 0;  // don't move until finger + bow have lifted
    ...
    bool bowLowerStarted = false;     // bow descent already begun (bow lead)
    double lastIntensity = -1.0;  // last intensity pushed to bow motor + press (live)
...
// Bow H-bridge pins, built parallel to g_profile.bows (each bow names its string,
// whose BOW_PWM<n>/BOW_DIR<n> lines it uses). The shared ENABLE is BOW_EN.
void buildBowPins(std::vector<BowPins>& out, int8_t& bowEnablePin) {
    bowEnablePin = pinOf("BOW_EN");
    out.assign(g_profile.bows.size(), BowPins{});
    for (size_t bi = 0; bi < g_profile.bows.size(); ++bi) {
        int s = g_profile.bows[bi].stringIndex;
        if (s < 0) continue;
        const std::string idx = std::to_string(s + 1);
        for (const auto& a : g_profile.pins) {
            if (a.signal == "BOW_PWM" + idx) out[bi].pwm = a.gpio;
            else if (a.signal == "BOW_DIR" + idx) out[bi].dir = a.gpio;
        }
    }
}
...
void applyProfile() {
    ...
    // Bow motors after the servos: the bow PWM shares the ESP32-S3 LEDC channels
    // with any direct-GPIO servos, so it allocates from the first free channel.
    std::vector<BowPins> bowPins;
    int8_t bowEnablePin = -1;
    buildBowPins(bowPins, bowEnablePin);
    g_bows.begin(g_profile.bows, bowPins, bowEnablePin, g_servos.directChannelsUsed());
...
void faultRuntimeAxis(size_t i, const char* reason, uint32_t nowMs) {
    ...
    // pressed and spinning on the string (leads can engage them before arrival).
    ...
    if (i < g_sched.size()) {
        ...
        int bp = g_servos.bowPressIndex(static_cast<int>(i));
        if (bp >= 0) g_servos.release(bp);      // lift the bow off the string
        int bm = g_bows.indexForString(static_cast<int>(i));
        if (bm >= 0) g_bows.stop(bm, nowMs);    // ramp the bow motor down
...
bool beginHoming(uint32_t nowMs) {
    ...
    g_bows.stopAll();              // no bowing while the carriages seek home
    g_bows.enable(false);          // bow H-bridges stay disabled until armed
...
void doHoming(uint32_t nowMs) {
    ...
    g_bows.enable(true);       // bow H-bridges may now drive (motors still idle)
    if (faulted > 0) {
...
void neutraliseAll() {
    ...
    g_steppers.enableDrivers(false);
    g_servos.neutraliseAll();
    g_bows.stopAll();       // hard-stop every bow motor now
    g_bows.enable(false);   // and cut the shared H-bridge ENABLE
...
bool doActivateProfile(const Profile& p, uint32_t nowMs) {
    ...
    g_steppers.stopAll();
    g_steppers.enableDrivers(false);
    g_bows.stopAll();            // stop every bow motor before the config changes
    g_bows.enable(false);
    for (auto& s : g_sched) s = StringSched{};
...
// Drive one string's mechanical sequence toward a bowed note: move to the fret,
// press the finger, then lower the bow and spin the motor — bowing continuously
...
void tickString(size_t i, uint32_t nowMs) {
    ...
    if (!tgt.active) {
        ...
        if (sch.phase != StringSched::WaitStopped) {
            ...
            // move must not let it finish travelling), stop the bow motor and lift
            // the bow, then lift the finger.
            g_steppers.stop(i);
            int fi = g_servos.fingerIndex(static_cast<int>(i));
            if (fi >= 0) g_servos.release(fi);
            ...
            int bp = sch.bowPressIndex >= 0 ? sch.bowPressIndex
                                            : g_servos.bowPressIndex(static_cast<int>(i));
            if (bp >= 0) g_servos.release(bp);   // lift the bow off the string
            int bm = sch.bowMotorIndex >= 0 ? sch.bowMotorIndex
                                            : g_bows.indexForString(static_cast<int>(i));
            if (bm >= 0) g_bows.stop(bm, nowMs); // ramp the bow motor down to silence
    ...
    if (sch.commandId != tgt.commandId) {
        ...
        // ReplaceOldest): stop the bow motor and lift the bow off the string, so a
        // sounding string is not dragged to a new fret while still being bowed.
        sch.fingerIndex = g_servos.fingerIndex(static_cast<int>(i));
        sch.bowPressIndex = g_servos.bowPressIndex(static_cast<int>(i));
        sch.bowMotorIndex = g_bows.indexForString(static_cast<int>(i));
        if (sch.bowMotorIndex >= 0) g_bows.stop(sch.bowMotorIndex, nowMs);
        if (sch.bowPressIndex >= 0) g_servos.release(sch.bowPressIndex);
        ...
        sch.bowLowerStarted = false;
        sch.lastIntensity = -1.0;
        // Lift the finger and WAIT for the finger AND bow to travel up before the
        // carriage moves, so nothing drags along the string (§16).
        sch.commandId = tgt.commandId;
        if (sch.fingerIndex >= 0) g_servos.release(sch.fingerIndex);
        uint32_t lift = 0;
        if (sch.fingerIndex >= 0 && g_servos.travelMs(sch.fingerIndex) > lift)
            lift = g_servos.travelMs(sch.fingerIndex);
        if (sch.bowPressIndex >= 0 && g_servos.travelMs(sch.bowPressIndex) > lift)
            lift = g_servos.travelMs(sch.bowPressIndex);
        sch.moveHoldUntilMs = nowMs + lift;
    ...
    switch (sch.phase) {
        case StringSched::ReleasingFinger:
            // Start moving only once the finger and bow have lifted AND the carriage
            // has fully stopped (a new note arriving during a cancel deceleration
            // must not issue a moveTo into a moving motor).
            if (static_cast<int32_t>(nowMs - sch.moveHoldUntilMs) >= 0 &&
        ...
        case StringSched::Settling: {
            // Bow lead: begin lowering the bow (descent servo) up to bowLeadMs before
            // the string is Ready, so the bow is already in contact when the note is
            // due (overlaps the descent with the finger settle). Skip for a merely-
            // prepared note — it must not press on the string through the pre-trigger
            // window; its bow lowers after trigger.
            if (!sch.bowLowerStarted && sc.willArmOnSettle() &&
                g_profile.midi.bowLeadMs > 0 && sch.bowPressIndex >= 0) {
                uint32_t settle = g_servos.settleMs(sch.fingerIndex);
                if ((nowMs - sch.phaseStartMs) + g_profile.midi.bowLeadMs >= settle) {
                    if (!g_servos.pressForce(sch.bowPressIndex, tgt.intensity)) {
                        faultRuntimeAxis(i, "bow press servo write failed", nowMs);
                        break;
                    }
                    sch.bowLowerStarted = true;
                }
            }
        ...
        case StringSched::Ready: {
            ...
            // Pre-lower the bow DURING the fixed-delay wait so it is in contact by
            // executeAtMs (covers the open-string / zero-settle case the settle-time
            // bow lead does not). Begins one bow-servo travel before executeAtMs.
            if (sch.bowPressIndex >= 0 && !sch.bowLowerStarted) {
                uint32_t liftMs = g_servos.travelMs(sch.bowPressIndex);
            ...
            // scheduled execution time (the bow has been descending during it).
            if (static_cast<int32_t>(nowMs - sch.executeAtMs) < 0) break;
            if (!sc.startBow(tgt.commandId)) break;
            // Start bowing: bow pressed at the note's force AND the motor spun up —
            // both scaled by intensity (velocity). CC7/CC11 keep modulating it while
            // it sounds (see the Bowing case).
            if (sch.bowPressIndex >= 0) g_servos.pressForce(sch.bowPressIndex, tgt.intensity);
            if (sch.bowMotorIndex >= 0)
                g_bows.setIntensity(sch.bowMotorIndex, tgt.intensity, nowMs);
            sch.lastIntensity = tgt.intensity;
            sch.phase = StringSched::Bowing;
            break;
        }
        case StringSched::Bowing:
            // Sustain: a bowed note keeps sounding until Note Off. Follow live
            // intensity changes (CC7/CC11) by re-driving the motor speed and the bow
            // contact force whenever the target intensity moves.
            if (tgt.intensity != sch.lastIntensity) {
                if (sch.bowPressIndex >= 0)
                    g_servos.pressForce(sch.bowPressIndex, tgt.intensity);
                if (sch.bowMotorIndex >= 0)
                    g_bows.setIntensity(sch.bowMotorIndex, tgt.intensity, nowMs);
                sch.lastIntensity = tgt.intensity;
            }
...
void setup() {
    ...
    if (!g_storage.load(g_storage.startupSlot(), g_profile) ||
        ...
        // Violin, standard tuning G3 D4 A4 E5 (MIDI 55/62/69/76).
        g_profile = Profile::makeDefault("Violin", 4, {55, 62, 69, 76}, 12);
    }
...
void loop() {
    ...
    if (wasConnected && !nowConnected && g_phase == AppPhase::Ready) {
        ...
        g_bows.stopAll();      // silence every bow motor on the controlled stop
    ...
    g_bows.update(nowMs);       // advance bow-motor spin-up / spin-down ramps

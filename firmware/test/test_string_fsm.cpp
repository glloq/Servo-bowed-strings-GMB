...
// A bowed note sounds continuously: Note Off from Bowing goes through Stopping
// (bow off + lifted) back to Idle, unlike a plucked note that just decays.
TEST(fsm_note_off_stops_a_sounding_note) {
    StringController c = armed();
    ...
    CHECK(c.state() == StringState::Bowing);
    c.noteOff();
    CHECK(c.state() == StringState::Stopping);
    c.stoppingDone();
...
TEST(fsm_note_off_cancels_prepared_bow) {
    ...
    c.noteOff();
    CHECK(!c.bowArmed());
    // The old deferred bow start must be ignored (stale command id). It was armed
    // (not yet sounding), so Note Off returns straight to Idle, not Stopping.
    CHECK(!c.startBow(id));
    CHECK(c.state() != StringState::Bowing);
    CHECK(c.state() == StringState::Idle);
}

// A stale command id from a replaced note must never start a bow.
...
// trigger() before the string settles arms on-settle: the bow starts the moment

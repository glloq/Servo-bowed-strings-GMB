...
TEST(validator_requires_bow_per_string) {
    Profile p = uke();
    // Remove every bow motor: no string can be sounded, so this must fail.
    p.bows.clear();
    CHECK(!ProfileValidator::isActivatable(p));
    // Restoring one bow motor per string satisfies the requirement.
    for (size_t i = 0; i < p.strings.size(); ++i) {
        BowConfig b;
        b.enabled = true;
        b.stringIndex = static_cast<int8_t>(i);
        p.bows.push_back(b);
    }
    CHECK(ProfileValidator::isActivatable(p));
}

TEST(validator_limits_ledc_channels_to_eight) {
    Profile p = uke();  // 4 bow motors already use 4 LEDC channels
    for (int i = 0; i < 9; ++i) {
    ...
    for (const auto& is : ProfileValidator::validate(p))
        if (is.field == "ledc.channels") over = true;
    CHECK(over);  // 4 bows + 9 direct servos = 13 > 8 LEDC channels
}
...
// --- P0/P1: a disabled axis does not require pins, a bow or a bowPress servo ---
TEST(disabled_axis_not_required_in_validation) {
    ...
    // Remove string 4's STEP/DIR/HOME and bow H-bridge pins.
    for (auto it = p.pins.begin(); it != p.pins.end();) {
        if (it->signal == "STEP4" || it->signal == "DIR4" || it->signal == "HOME4" ||
            it->signal == "BOW_PWM4" || it->signal == "BOW_DIR4")
            it = p.pins.erase(it);
        else ++it;
    }
    // Remove string 4's finger + bowPress servos and its bow motor.
    for (auto it = p.servos.begin(); it != p.servos.end();) {
        if (it->stringIndex == 3) it = p.servos.erase(it);
        else ++it;
    }
    for (auto it = p.bows.begin(); it != p.bows.end();) {
        if (it->stringIndex == 3) it = p.bows.erase(it);
        else ++it;
    }

...
TEST(auto_assign_matches_recommended_table) {
    ...
    PinManager pm(b);
    PinRequest req;
    req.stringCount = 4;  // a bowed instrument tops out at four strings
    ...
    // Bow H-bridge lines: PWM + DIR per string, one shared ENABLE.
    CHECK_EQ(pm.gpioOf("BOW_PWM1"), 1);
    CHECK_EQ(pm.gpioOf("BOW_DIR1"), 15);
    CHECK_EQ(pm.gpioOf("BOW_DIR4"), 39);
    CHECK_EQ(pm.gpioOf("BOW_EN"), 33);
